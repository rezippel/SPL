...
(defun make-spl-window (&key (parent *lisp-main-window*) 
   ...
   (let (window-0 window window-2 window-3 window-4)
      (setq window-0 
            ...
            (if 
               (or (not (pathnamep pathname)) 
                  (member (pathname-type pathname) *fsl-extensions* 
                     :test #'string-equal))
               "C:\\rz\\Simlab\\spl\\src\\SPL-Window" 
               (namestring pathname))))
      ...
      (setf (window-editable-p window) t)
      (setf (getf (stream-plist window) :path) 
         (let* ((pathname (load-time-value *load-pathname*)))
            (if 
               (or (not (pathnamep pathname)) 
                  (member (pathname-type pathname) *fsl-extensions* 
      ...
      (setf (getf (stream-plist window) :startup-state) nil)
      (setf (getf (stream-plist window) :top-level-p) nil)
      (setf (help-string window) (delete #\Newline nil))
      (setf (getf (stream-plist window) :package) nil)
      (add-toolbar-to-window window-0 window)
      (setq window (frame-child window-0))
